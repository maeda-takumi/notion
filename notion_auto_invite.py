...
def load_shinchoku_target(config_path: Path) -> Optional[ShinchokuTarget]:
    ...
    return ShinchokuTarget(
        ...
        name_column=int(cfg["name_column"]) if cfg.get("name_column") else None,
...
def update_shinchoku_sheet(
    ...
    for row_index, line_name in enumerate(sheet.col_values(5), start=1):
        normalized = line_name.strip()
        if normalized and normalized not in line_name_to_row:
            line_name_to_row[normalized] = row_index

    processed_line_names = set()
    today = datetime.now().strftime("%Y/%m/%d")
    date_column = progress_column - 1

    for invite in invites:
        line_name = invite.line_name.strip()
        if not line_name:
            if log_callback:
                log_callback(f"{target_name}: LINE名未設定のため進捗記録をスキップ email={invite.email}")
            continue

        if line_name in processed_line_names:
            continue

        target_row = line_name_to_row.get(line_name)
        if target_row is None:
            if log_callback:
                log_callback(f"{target_name}: shichokuシートでLINE名が見つからないためスキップ LINE名={line_name}")
            continue

        sheet.update_cell(target_row, date_column, today)
        sheet.update_cell(target_row, progress_column, "済")
        processed_line_names.add(line_name)
        if log_callback:
            log_callback(
                f"{target_name}: shichoku更新 row={target_row}, LINE名={line_name}, 日付列={date_column}, 進捗列={progress_column}"
            )
    ...
    for target_name, target in targets.items():
        ...
        for pending in pending_invites:
            ...
            if shinchoku_row is None:
                if log_callback:
                    log_callback(
                        f"{target_name}: shichokuシートでLINE名が見つからないためスキップ LINE名={pending.name}"
                    )
                continue

            shinchoku_sheet.update_cell(shinchoku_row, shinchoku_status_column, target.invited_text)
            if log_callback:
                log_callback(
                    f"{target_name}: shichoku更新 row={shinchoku_row}, col={shinchoku_status_column}, LINE名={pending.name}"
                )
